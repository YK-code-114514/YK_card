...
varying vec2 vUv;
uniform float uTime, uFoil, uScale, uDepth, uBgDepth, uTextDepth, uFinish, uHasLine, uRelief;
uniform vec2 uFit;
uniform vec2 uShadowDir;
uniform float uShadowDepth, uShadowOpacity;
uniform vec2 uOffset;
uniform float uClip;
uniform float uFlash, uFlashIntensity, uFlashScale, uFlashSpeed;
uniform vec3 uView;
...
void main() {
  vec2 uv = vUv;
  vec2 su = (parallax(uv,uDepth) - .5) * uScale / uFit + .5 + uOffset;
  vec2 bu = parallax(uv,uBgDepth);
  vec4 subject = texture2D(tSubject,clamp(su,0.,1.));
  subject.a *= (uClip < .5 ? 1. : inside(su)) * (1.-uRelief);
  vec3 bg = texture2D(tBackground,clamp(bu,0.,1.)).rgb;
  vec3 col = bg;
  vec2 shuv = uv + uShadowDir * uShadowDepth + uView.xy * .02;
  vec4 sh = texture2D(tShadow, shuv);
  col = mix(col, vec3(0.), sh.a * uShadowOpacity);
  col = mix(col, subject.rgb, subject.a);
  // 动态闪光：主体上的四芒星星芒闪烁 + 柔和扫光（呼应闪光海报效果）
  if (uFlash > .5 && uFlashIntensity > 0.) {
    vec2 gp = su * (10.0 * uFlashScale);
    vec2 cell = floor(gp);
    vec2 f = fract(gp) - .5;
    float h = hash(cell);
    float sp = uFlashSpeed * (0.35 + h * .8);
    float phase = fract(h * 7.13 + uTime * sp);
    float tw = pow(smoothstep(.90, 1.0, phase), 3.);
    float star = smoothstep(.42, .0, abs(f.x)) * smoothstep(.22, .0, abs(f.y));
    star += smoothstep(.42, .0, abs(f.y)) * smoothstep(.22, .0, abs(f.x));
    star = clamp(star, 0., 1.);
    float spark = star * tw * uFlashIntensity * (0.35 + 0.65 * h);
    vec3 sc = mix(vec3(1., .94, .78), vec3(.86, .95, 1.), step(.5, h));
    col += sc * spark * subject.a;
    float bnd = smoothstep(.0, .22, abs(fract(uTime * uFlashSpeed * .12 + (su.x + su.y) * .6) - .5));
    col += vec3(1., .94, .8) * (1. - bnd) * .05 * uFlashIntensity * subject.a;
  }
  ...
  vec4 text = texture2D(tText,parallax(uv,uTextDepth));
    ...
    const tree =
      ...
      icons[
        name.charAt(0).toUpperCase() +
          name.slice(1).replace(/-([a-z0-9])/g, (m, c) => c.toUpperCase())
      ];
    if (!tree) return;
...
async function init() {
  ...
  document.title = config.title + " · 闪卡工坊";
  for (const [id, key] of [
    ["card-title", "title"],
    ["subtitle", "subtitle"],
  ...
  uniforms = {
    ...
    tShadow: { value: canvasTexture(blankShadowCanvas()) },
    ...
    uTextDepth: { value: 0 },
    uShadowDir: { value: new THREE.Vector2(1, 1).normalize() },
    uShadowDepth: { value: 0.02 },
    uShadowOpacity: { value: 0.45 },
    uOffset: { value: new THREE.Vector2(0, 0) },
    uClip: { value: 1 },
    uFlash: { value: 1 },
    uFlashIntensity: { value: 0.8 },
    uFlashScale: { value: 1.0 },
    uFlashSpeed: { value: 1.0 },
    uFinish: { value: 0 },
  ...
  window.__holo = {
    ...
    makeCanvasTexture: canvasTexture,
    makeVector2: (x, y) => new THREE.Vector2(x, y),
    layers: reliefLayers,
